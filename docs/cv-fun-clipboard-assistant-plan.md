...
`cv-fun` 的核心目标是做一个面向 macOS/Windows 的剪贴板效率工具。第一阶段完整实现类似 Maccy 的剪贴板历史能力：记录用户复制过的文本，提供快速搜索、选择、复制、粘贴、置顶、删除和清理。第二阶段在历史能力之上增加“复制内容解析”：当用户复制时间戳、日期、英文单词、URL、JSON、颜色值等文本时，自动识别类型并在面板中展示可直接使用的解析结果。
...
当前 `cv-fun` 是 Wails + Vue 项目。建议把后端能力按领域拆分，避免 `main.go` 继续变胖。
